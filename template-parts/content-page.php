...
<article class="post post-full card bg-white shadow-sm border-0" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="post-meta">
			<?php
				...
				for ($i = 0; $i < count($metaList); $i++){
					...
					echo $template::get_article_meta($metaList[$i]);
				}
			?>
			<?php if (!post_password_required() && $options->get("show_readingtime") != "false" && $template::is_readingtime_meta_hidden() == False) {
				echo $template::get_article_reading_time_meta(get_the_content());
			} ?>
		</div>
		<?php
			if ($template::has_post_thumbnail() && $options->get('show_thumbnail_in_banner_in_content_page') != 'true'){
	...
	<?php
		$referenceList = \ArgonModern\Shortcodes::get_reference_list();
		if ($referenceList != ""){
			echo $referenceList;
		}
	?>
